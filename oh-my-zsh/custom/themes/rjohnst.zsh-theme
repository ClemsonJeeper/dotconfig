...
prompt_context() {
  _host="%n@%m"
  if [[ -n "$BAAS_VOLUME_NAME" ]]; then
    _host="%n@${BAAS_VOLUME_NAME}"
  fi
  if [[ "$USERNAME" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment "$AGNOSTER_CONTEXT_BG" "$AGNOSTER_CONTEXT_FG" "%(!.%{%F{$AGNOSTER_STATUS_ROOT_FG}%}.)${_host}"
  fi
}
